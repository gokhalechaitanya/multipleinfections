...
Indeed, a stickleback can be infected by numerous parasites, all vying for control, as shown above and photographed by Martin Kalbe \citep{kalbe:JFB:2002}.
}
\label{fig:empirical}
\end{figure}
...
Host manipulation in trophically transmitted parasites, therefore, affects not only the intermediate host population but also the entire predator-prey dynamics. 
...
These intermediate hosts are then predated upon by the definitive hosts, thus moving the parasites to the final host (either as $D_w$ or $D_{ww}$) where they can reproduce and reenter the free-living stage in the environmental pool $W$.
}
\label{fig:schematic}
\end{figure}
...
This formulation assumes that infection always happens when intermediate hosts encounter free-living parasites and when definitive hosts consume infected intermediate hosts (Fig.~\ref{fig:schematic}).
...
Here, as reproduction and transmission value of the parasite are not sufficient, it goes extinct (Figure \ref{fig:ecotraject:nonlinear}A), leaving the predator-prey dynamics attaining equilibrium (Figure \ref{fig:ecotraject:nonlinear}B)
...
\cha{
On the left, we show the density dynamics of the susceptible intermediate and definitive hosts. 
The top shows the case of a disease-free equilibrium being reached from different initial conditions (ic). The bottom shows the case where the parasite also survives.
The detailed dynamics of all different compartments are further shown for specific initial conditions (ic2), including the free-living parasite dynamics.
}
...
Specifically, if sufficient parasites are introduced into the disease-free predator-prey populations, the parasite population persists and reaches a stable equilibrium (Figure \ref{fig:bistability}B, C). 
In contrast, if only a few parasites are introduced into the disease-free populations, or if sufficient disturbance occurs when the parasite population is already established, the parasite population could go extinct (Figure \ref{fig:bistability}B, C).
...
We often expect intermediate levels of coordination where a bistable area could occur (top left quadrant in Figure \ref{fig:manipbifur}A, C, D).
However, the size of this area is sensitive to the value of reproduction and manipulation in a single infection. 
In particular, higher values of these two parameters reduce the bistability area so that sufficiently large reproduction in a single infection can guarantee single equilibrium when parasites coordinate (Figure \ref{fig:manipbifur} B).
...
Moreover, the results of our ecological model are a baseline for further investigation of the evolution of host manipulation, where introducing the parasite pool may create interesting eco-evolutionary feedback to the system.

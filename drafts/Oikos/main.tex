...
A) A bifurcation graph for different values of reproduction in single infection and double infections. B) Equilibrium values of intermediate host when $\epsilon = 4$ when bistability occur at high values of $f_w$ (in pink), and $\epsilon = 4$ when only one stable equilibrium exist at high values of $f_w$ (in purple). 
C) Details of the parasites population dynamics in the case of bistability shown through the infected intermediate hosts. 
When the parasites start at high density, the parasite population persists whereas when they start at lower density, the parasite population perishes.

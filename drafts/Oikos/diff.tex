...
%DIF ADD Oikos_main.tex               Thu Mar 14 11:34:56 2024
...
\DIFdelbegin \DIFdel{Although manipulation correlates with the transmissionrate in trophically transmitted parasites and infectious diseases}\DIFdelend \DIFaddbegin \color{red} \DIFadd{Even though host manipulation and virulence both correlates with parasite transmission}\DIFaddend , there are \DIFdelbegin \DIFdel{differences}\DIFdelend \DIFaddbegin \DIFadd{subtle differences, such that virulence implies an addition to the natural mortality rate of the infected host, while manipulation links to immediate death of the intermediate host due to predation}\DIFaddend . 
Host manipulation \DIFdelbegin \DIFdel{influences the predation rate in trophically transmitted parasites, predominantly affecting }\DIFdelend \DIFaddbegin \DIFadd{therefore not only affects the intermediate host population but the entire }\DIFaddend predator-prey dynamics. 
Theoretical studies on host manipulation in trophically transmitted parasites \DIFdelbegin \DIFdel{with multiple infections are rare \mbox{%DIFAUXCMD
\citep{Parker2003,Vickery2009}}\hskip0pt%DIFAUXCMD
. 
Moreover, they do not consider }\DIFdelend \DIFaddbegin \DIFadd{rarely consider multiple infections and those that did incorporate this feature neglect }\DIFaddend the prey-predator dynamics, which will likely have important feedback on the evolution of host manipulation \DIFdelbegin \DIFdel{. 
...
%DIF > They show multiple infections can increase virulence \citep{vanBaalen1995, Choisy2010}.
%DIF > Evolutionary branching of a less virulent and a hypervirulent parasite can occur when considering within-host dynamics \citep{ Alizon2008}, and a reduction in virulence is possible if parasites are co-transmitted \citep{Alizon2012}. 
%DIF > These studies also involve host manipulation to a certain extent, as it can affect transmission rates, even though they do not explicitly consider the trait. 
%DIF > Host manipulation in trophically transmitted parasites receives less attention. 
%DIF > Although manipulation correlates with the transmission rate in trophically transmitted parasites and infectious diseases, there are differences. 
%DIF > Host manipulation influences the predation rate in trophically transmitted parasites, predominantly affecting predator-prey dynamics. 
%DIF > Theoretical studies on host manipulation in trophically transmitted parasites with multiple infections are rare \citep{Parker2003,Vickery2009}. 
%DIF > Moreover, they do not consider the prey-predator dynamics, which will likely have important feedback on the evolution of host manipulation. 
%DIF > A few studies considering the prey-predator dynamics do not incorporate multiple infections \citep{Rogawa2018, Iritani2018, Hadeler1989, Fenton2006}. 
%DIF > More importantly, they assume that transmission from definitive hosts to intermediate hosts is due to direct contact between the two types of hosts. 
%DIF > This is often not the case, as parasites are released from the definitive hosts into the environment. 
%DIF > Transmission happens only when intermediate hosts have contact with this free-living parasite pool.
}\DIFaddend 


\DIFaddbegin \color{black}\DIFaddend Our study addresses the gap in the theoretical work on host manipulation in trophically transmitted parasites.
...
\DIFaddendFL \caption{\DIFaddbeginFL \DIFaddFL{\cha{\textbf{title}}
...
Right: Changes of the bistability area \DIFdelbeginFL \DIFdelFL{(}\DIFdelendFL shaded areas) \DIFdelbeginFL \DIFdelFL{with respect to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{concerning }\DIFaddendFL different reproduction rates in single infection (different boundary styles). 

...
%DIF DEL old/Oikos_submittedversion_round1.tex   Fri Jun 14 08:53:26 2024
%DIF ADD main.tex                                Tue Jul  2 09:14:41 2024
...
}\DIFdelend %DIF > Thus, the reduction in mobility is suggested to reduce the predation rate by the definitive hosts. 
...
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.75\columnwidth]{Figures/Sept_10_ 101.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.5\columnwidth]{Figures/Sept_10.pdf}
\DIFaddendFL \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Who is in control?.}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Who is in control?}}
\DIFaddendFL Schistocephalus eggs \DIFdelbeginFL \DIFdelFL{, which overwinter at the bottom of bodies of water, }\DIFdelendFL hatch into microscopically tiny swimming larvae. 
These larvae are eaten by copepods\DIFdelbeginFL \DIFdelFL{(also known as Cyclops due to its single eye)}\DIFdelendFL , where they develop to the second larval stage. 
...
The \DIFdelbeginFL \DIFdelFL{timing is crucial as the chances of success are greatest if the larvae develop in the copepod for 13 to 15 days before entering the stickleback. 
The }\DIFdelendFL presence of multiple parasites in the same host can lead to competition and strategic decisions pertaining to investment in manipulation and growth.
\DIFdelbeginFL \DIFdelFL{And indeed}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Indeed}\DIFaddendFL , a stickleback can be infected by numerous \DIFdelbeginFL \DIFdelFL{tapeworms}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{parasites}\DIFaddendFL , \DIFaddbeginFL \DIFaddFL{all vying for control, }\DIFaddendFL as shown \DIFdelbeginFL \DIFdelFL{above }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and photographed }\DIFaddendFL by Martin Kalbe \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\citep{kalbe:JFB:2002}}\hspace{0pt}%DIFAUXCMD
}\DIFaddendFL .
}
\label{fig:empirical}
\end{figure}
...
\caption{\textbf{\DIFaddFL{Schematics of the transmission routes.}} 
\DIFaddFL{Blue ovals represent the intermediate hosts, while green diamonds represent the definitive hosts.
...
\DIFaddend For simplicity, we assume that hosts can be infected by one (single infection) or, at most, two parasites (double infections).
...
\DIFdelbegin \DIFdel{Given that infection occurs}\DIFdelend \DIFaddbegin \DIFadd{Figure (\ref{fig:schematic}) illustrates the transmission dynamics, and details of the model's variables and parameters are shown in Table \ref{table:varpardescription}.
Note that multiple infections in nature often involve more than two parasites.
Typically, the number of parasites in multiple infections follows a negative binomial distribution, i.e. most hosts are infected with a few parasites while very few hosts are infected with many parasites \mbox{%DIFAUXCMD
\citep{wilson:FE:1996}}\hspace{0pt}%DIFAUXCMD
.
...
%DIF > Furthermore, what matters is the magnitude of the manipulative effect in multiple infections versus in single infection.
Instead, we focus on another aspect of multiple infections, that is, co-transmission, which has been shown to affect the evolutionary trajectories of parasites in infectious disease \mbox{%DIFAUXCMD
\citep{Alizon2012}}\hspace{0pt}%DIFAUXCMD
.
Given an infection}\DIFaddend , the probability that two parasites from the parasite pool co-transmit to an intermediate host is denoted by $p$. 
...
This formulation assumes that infection always happens when \DIFdelbegin \DIFdel{hosts encounter parasites.
}\DIFdelend \DIFaddbegin \DIFadd{intermediate hosts encounter free-living parasites and when definitive hosts consume infected intermediate hosts (Figure.~\ref{fig:schematic}).
}\DIFaddend The dynamics of a complex life cycle parasite that requires two host species is described by the following system of equations, firstly for the intermediate host as,
%
\begin{align}
\frac{dI_s}{dt} &= R(I\DIFdelbegin \DIFdel{_s, I_w, I_{ww}}\DIFdelend \DIFaddbegin \DIFadd{_{total}}\DIFaddend ) - d I_s - P_s(D\DIFdelbegin \DIFdel{_s, D_w, D_{ww}}\DIFdelend \DIFaddbegin \DIFadd{_{total}}\DIFaddend ) I_s  - \eta  I_s \nonumber \\ 
\frac{dI_w}{dt} &=  (1 - p) \eta I_s  - (d + \alpha_w) I_w - P_w(D\DIFdelbegin \DIFdel{_s}\DIFdelend \DIFaddbegin \DIFadd{_{total}}\DIFaddend , \DIFdelbegin \DIFdel{D_w, D_{ww}, }\DIFdelend \beta_w) I_w \label{odes:ihosts} \\
\frac{dI_{ww}}{dt} &= p \eta I_s  - (d + \alpha_{ww}) I_{ww} - P_{ww}(D\DIFdelbegin \DIFdel{_s}\DIFdelend \DIFaddbegin \DIFadd{_{total}}\DIFaddend , \DIFdelbegin \DIFdel{D_w, D_{ww}, }\DIFdelend \beta_{ww}) I_{ww} \nonumber
\end{align}
%
where \DIFdelbegin \DIFdel{$R(I_s, I_w, I_{ww})$ }\DIFdelend \DIFaddbegin \DIFadd{$R(I_{total})$ }\DIFaddend represents the birth rate of the intermediate hosts, a function of both infected and uninfected individuals \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{$I_{total} = I_s + I_w + I_{ww}$.
...
}\DIFaddend $P_s, \ P_w, \ P_{ww}$ are the predation functions of definitive hosts on susceptible, singly infected and doubly infected intermediate hosts. 
The predation function depends on the density of \DIFdelbegin \DIFdel{the definitive hosts }\DIFdelend \DIFaddbegin \DIFadd{all definitive hosts $D_{total} = D_s + D_w + D_{ww}$ }\DIFaddend and the manipulative strategies of parasites in the intermediate hosts. 
...
\DIFdelbegin \DIFdel{In the scope of this model, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend assume no specific relationship between $\beta_w$ and $\beta_{ww}$ to explore all possible ecological outcomes of the system. 
The force of infection by parasites in the environment is denoted by $\eta = \gamma W$\DIFaddbegin \DIFadd{, where $\gamma$ represents the infection rate of free-living parasites}\DIFaddend . 
...
%
\begin{align}
\frac{dD_s}{dt} &= B(D\DIFdelbegin \DIFdel{_s}\DIFdelend \DIFaddbegin \DIFadd{_{total}}\DIFaddend ,  \DIFdelbegin \DIFdel{D_w,  D_{ww},  }\DIFdelend I\DIFdelbegin \DIFdel{_s, I_w, I_{ww}}\DIFdelend \DIFaddbegin \DIFadd{_{total}}\DIFaddend )  - \mu D_s - (\lambda_{ww} + \lambda_w) D_s \nonumber \\    
...
\end{align}
%
where \DIFdelbegin \DIFdel{$B(D_s, D_w, D_{ww}, I_s, I_w, I_{ww})$ }\DIFdelend \DIFaddbegin \DIFadd{$B(D_{total},  I_{total})$ }\DIFaddend represents the birth rate of definitive hosts.
The birth rates depend on the density of both intermediate and definitive hosts, infected or uninfected. 
The \DIFaddbegin \DIFadd{natural mortality rate of definitive hosts is represented by $\mu$, and parasites induce additional mortality rates $\sigma_w$ and $\sigma_{ww}$ in single and double infection, respectively.
The }\DIFaddend force of infection that corresponds respectively to singly infected intermediate host ($I_w$) and doubly infected intermediate hosts ($I_{ww}$) is denoted respectively by $\lambda_w = h (\rho + \beta_w)  I_w$ and $\lambda_{ww} = h (\rho + \beta_{ww}) I_{ww}$, where $\rho$ is the baseline predation rate, i.e. the basic constitutive level of predation, and $h$ is the probability that the parasite successfully establishes inside the host.
Without manipulation, that is, $\beta_w = \beta_{ww} = 0$, the parasite is still transmitted via the baseline predation $\rho$. 
The dynamics of the free-living parasites in the environment are then given by\DIFaddbegin \DIFadd{,
}\DIFaddend \begin{align}
	\frac{dW}{dt} &= f_w D_w + f_{ww} D_{ww} - \delta W - \eta I_s
	\DIFdelbegin \DIFdel{. }\DIFdelend \label{odes:eparasite}
\end{align}
%DIF < 
\DIFaddbegin \DIFadd{where $f_w$ and $f_{ww}$ are the reproduction rates of parasites in single and double infection, respectively, and parasites die naturally at a rate $\delta$.
}\DIFaddend 
...
%DIFDELCMD < \captionsetup{format=plain}
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\textwidth]{Figures/schematic.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\section*{\DIFdelbegin \DIFdel{Results}\DIFdelend \DIFaddbegin \DIFadd{Basic reproduction ratio $R_0$ of the parasites}\DIFaddend }
\DIFdelbegin \subsection*{\DIFdel{Basic reproduction ratio $R_0$ of the parasites}}
%DIFAUXCMD
\DIFdelend 
...
\DIFdelbegin \subsection*{\DIFdel{Birth function of intermediate hosts}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \section*{\DIFadd{Birth function of intermediate hosts}}
\DIFaddend 

The simplest form of the prey's birth rate is a linear function, in which case the disease-free equilibrium is always \DIFdelbegin \DIFdel{unstable. In particular, it has a cyclic behaviour because, at this equilibrium, the jacobian matrix of the system (\ref{odes:ihosts}, \ref{odes:dhosts}, \ref{odes:eparasite}) always has two pure imaginary eigenvalues (see SI2}\DIFdelend \DIFaddbegin \DIFadd{in a cyclic regime (see SI 2}\DIFaddend ).
...
\DIFdelbegin \DIFdel{Here}\DIFdelend \DIFaddbegin \DIFadd{Therefore}\DIFaddend , we choose a non-linear birth function of the intermediate hosts to obtain a stable disease-free state and focus on the effect of host manipulation on the ecological dynamics (Figure \ref{fig:ecotraject:nonlinear}). 
...
\begin{figure}[!ht]
\captionsetup{format=plain}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Figures/ecotraject_nonlinear.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\textwidth]{Figures/ecotraject_nonlinear.pdf}
\DIFaddendFL \caption{\textbf{Ecological dynamics of the predator-prey-parasite system.} 
\DIFdelbeginFL \DIFdelFL{A) Ecological trajectories of infected hosts and free-living parasite when parasites cannot persist}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{On the left}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{B) Phase plane }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{we show the density dynamics }\DIFaddendFL of \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL susceptible intermediate and definitive hosts \DIFdelbeginFL \DIFdelFL{under disease free scenario. C}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{at different initial conditions (ic1, ic2, and ic3}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{Ecological trajectories }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. 
The detailed dynamics }\DIFaddendFL of infected \DIFdelbeginFL \DIFdelFL{hosts and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{compartments are further shown for specific initial conditions (ic2), including the }\DIFaddendFL free-living parasite \DIFdelbeginFL \DIFdelFL{when parasites persist}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{dynamics}\DIFaddendFL .
\DIFdelbeginFL \DIFdelFL{D}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{A-B}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{Phase plane }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{A case }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{susceptible and definitive host under disease circulating scenario}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a disease-free equilibrium being reached from different initial conditions (ic)}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{C-D) A case where the parasite survives.
...
\DIFaddbegin \begin{figure}[!ht]
	...
	\caption{\textbf{\DIFaddFL{Effect of manipulation in single and double infections on the reproduction ratio $R_0$.}}
		\DIFaddFL{$R_0$ values increase with more efficient manipulation in single and double infection.  
...
\DIFaddend Our numerical results show that the parasite reproduction is substantial compared to other parameters (Figure \ref{fig:bistability}A). 
For instance, in the parameter set used to generate Figure \DIFdelbegin \DIFdel{(\ref{fig:bistability}B}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:bistability}}\DIFaddend , to spread in the predator-prey system, the value of parasite reproduction ($f_w$) has to be at least  20 times the value of intermediate host reproduction $r = 2.5$, given that both these parameters represent the \textit{per capita} growth rate of the parasite and the intermediate host population.
This observation suggests that trophically transmitted parasites \DIFdelbegin \DIFdel{must release a large amount of }\DIFdelend \DIFaddbegin \DIFadd{should release many }\DIFaddend offspring into the environment to persist. 
Interestingly, bistability occurs if the reproduction rate of the parasite in double infections is enhanced\DIFdelbegin \DIFdel{(Figure \ref{fig:bistability}A). 
The parasite population can reach }\DIFdelend \DIFaddbegin \DIFadd{. 
Bistability suggests that the parasite population is vulnerable to extinction. 
Specifically, if sufficient parasites are introduced into the disease-free predator-prey populations, the parasite population persists and reaches }\DIFaddend a stable equilibrium\DIFdelbegin \DIFdel{in the bistable region if the initial density is large enough}\DIFdelend . 
In contrast, \DIFdelbegin \DIFdel{with sufficient disturbance }\DIFdelend \DIFaddbegin \DIFadd{if only a few parasites are introduced into the disease-free populations, or if sufficient disturbance occurs when the parasite population is already established}\DIFaddend , the parasite population could go extinct \DIFaddbegin \DIFadd{(Figure \ref{fig:bistability}C)}\DIFaddend .

\begin{figure}[!ht]
\captionsetup{format=plain}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = \textwidth]{Figures/reproduction_bifurcation.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = \textwidth]{Figures/reproduction_bifurcation2.pdf}
\DIFaddendFL \caption{\textbf{Effect of parasite reproduction on the ecological dynamics}. 
A) \DIFdelbeginFL \DIFdelFL{Enhanced }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{A bifurcation graph for different }\DIFaddendFL reproduction \DIFaddbeginFL \DIFaddFL{values }\DIFaddendFL in \DIFaddbeginFL \DIFaddFL{single and }\DIFaddendFL double \DIFdelbeginFL \DIFdelFL{infection leads to bistability, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{infections. 
}\DIFaddendFL B\DIFdelbeginFL \DIFdelFL{, C}\DIFdelendFL ) \DIFdelbeginFL \DIFdelFL{Density }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Equilibrium density }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{singly infected }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{intermediate }\DIFaddendFL host \DIFdelbeginFL \DIFdelFL{at equilibrium }\DIFdelendFL when \DIFdelbeginFL \DIFdelFL{reproduction }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\epsilon = 4$ when bistability occurs at high values }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{parasites are the same }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_w$ (}\DIFaddendFL in \DIFdelbeginFL \DIFdelFL{singly and doubly infected hosts $f_{ww} = f_w$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pink)}\DIFaddendFL , and \DIFaddbeginFL \DIFaddFL{$\epsilon = 4$ }\DIFaddendFL when \DIFdelbeginFL \DIFdelFL{reproduction }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{only one stable equilibrium exists at high values }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{parasites }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_w$ (}\DIFaddendFL in \DIFdelbeginFL \DIFdelFL{doubly infected hosts is enhanced four times than those }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{purple). 
C) Details of the parasite population dynamics }\DIFaddendFL in \DIFdelbeginFL \DIFdelFL{singly }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the case of bistability shown through the }\DIFaddendFL infected \DIFaddbeginFL \DIFaddFL{intermediate }\DIFaddendFL hosts\DIFdelbeginFL \DIFdelFL{$f_{ww} = 4 f_w$}\DIFdelendFL . 
\DIFaddbeginFL \DIFaddFL{When the parasites start at high density, the parasite population persists, whereas when they start at lower density, they perish.
}\DIFaddendFL Filled circles indicate stable equilibrium\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL and open circles indicate unstable equilibrium. Parameter $\rho = 1.2, \  d = 0.9, \  r = 2.5, \ \gamma = 2.9, \ \alpha_w = 0, \ \alpha_{ww} =  0, \ \beta_w = 1.5, \ \beta_{ww} = 1.5, \ p = 0.05, \  c = 1.4, \ \mu = 3.9,  \ \sigma_w = 0, \ \sigma_{ww} = 0, \  q = 0.05, \ \delta = 0.9, \ k = 0.26, \ h = 0.6$.
}
...
Cooperation in parasite manipulation increases the parasite's basic reproduction ratio $R_0$, but the manipulation in a single infection substantially affects the value of $R_0$ (Figure \DIFdelbegin \DIFdel{\ref{fig:manipbifur}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:R0}}\DIFaddend ). 
...
However, \DIFdelbegin \DIFdel{suppose }\DIFdelend \DIFaddbegin \DIFadd{we could suppose that }\DIFaddend the ability to manipulate the host in a single infection is \DIFdelbegin \DIFdel{merely }\DIFdelend enough for the parasite population to escape extinction. 
In that case, the system is in a bistable state where intermediate cooperation in host manipulation cannot guarantee a single equilibrium (Hatched area Figure \DIFdelbegin \DIFdel{\ref{fig:manipbifur} Left}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:R0}}\DIFaddend ). 
...
Parasites can have an enhanced reproduction rate in coinfections, i.e. $f_{ww} > f_w$ (upper part of the horizontal line in \DIFdelbegin \DIFdel{Figure \ref{fig:manipbifur} Right}\DIFdelend \DIFaddbegin \DIFadd{all panels Figure \ref{fig:manipbifur}}\DIFaddend ). 
Likewise, they can compete for resources, so reproduction in double infection is suppressed compared to single infection (lower \DIFdelbegin \DIFdel{part }\DIFdelend \DIFaddbegin \DIFadd{parts }\DIFaddend of the horizontal \DIFdelbegin \DIFdel{line in Figure \ref{fig:manipbifur} Right}\DIFdelend \DIFaddbegin \DIFadd{lines in all panels Figure \ref{fig:manipbifur}}\DIFaddend ). 
Without any assumption on the \DIFdelbegin \DIFdel{relationship }\DIFdelend \DIFaddbegin \DIFadd{link }\DIFaddend between manipulative ability and reproduction, \DIFaddbegin \DIFadd{and a linear relationship between manipulation in single and double infections, }\DIFaddend we explore all possible combinations of cooperation-sabotage range in manipulation and suppressed-enhanced range in reproduction. This results in four scenarios of parameter combinations: i, parasites sabotage manipulation but have enhanced reproduction -- manipulative incoordination (top left \DIFdelbegin \DIFdel{quadrant in Figure \ref{fig:manipbifur} Right}\DIFdelend \DIFaddbegin \DIFadd{quadrants in all panels Figure \ref{fig:manipbifur}}\DIFaddend ), ii, parasites cooperate to increase manipulation and enhance reproduction -- coordination (top right \DIFdelbegin \DIFdel{quadrant in Figure \ref{fig:manipbifur} Right}\DIFdelend \DIFaddbegin \DIFadd{quadrants in all panels Figure \ref{fig:manipbifur}}\DIFaddend ), iii, parasites cooperate in manipulation but suppress reproduction -- reproductive incoordination (bottom right \DIFdelbegin \DIFdel{quadrant in Figure \ref{fig:manipbifur} Right}\DIFdelend \DIFaddbegin \DIFadd{quadrants in all panels Figure \ref{fig:manipbifur} }\DIFaddend ), and iv, parasites sabotage manipulation and suppress reproduction -- discordance (bottom left \DIFdelbegin \DIFdel{quadrant in Figure \ref{fig:manipbifur} Right}\DIFdelend \DIFaddbegin \DIFadd{quadrants in all panels Figure \ref{fig:manipbifur} }\DIFaddend ).

If coinfected parasites are discordant, i.e. uncooperative in manipulations and show suppressed reproduction, they cannot persist (\DIFdelbegin \DIFdel{Figure \ref{fig:manipbifur}}\DIFdelend \DIFaddbegin \DIFadd{bottom left quadrants Figure \ref{fig:manipbifur}A-D}\DIFaddend ). 
On the other extreme, where they are highly cooperative in manipulation and show enhanced reproduction, i.e., an extreme level of coordination, there is a guaranteed single equilibrium for parasite existence \DIFaddbegin \DIFadd{(top right quadrants Figure \ref{fig:manipbifur}A-D)}\DIFaddend . 
...
\DIFdelbegin \DIFdel{Very often , we }\DIFdelend \DIFaddbegin \DIFadd{We often }\DIFaddend expect intermediate levels of coordination where a bistable area could occur (top \DIFdelbegin \DIFdel{left }\DIFdelend \DIFaddbegin \DIFadd{right }\DIFaddend quadrant in Figure \ref{fig:manipbifur}\DIFdelbegin \DIFdel{at $f_w = (37, 37.5)$}\DIFdelend \DIFaddbegin \DIFadd{A, C, D}\DIFaddend ).
However, the size of this area is sensitive to the value of reproduction and manipulation in a single infection. 
In particular, higher values of these two parameters reduce the bistability area \DIFdelbegin \DIFdel{to the point }\DIFdelend \DIFaddbegin \DIFadd{so }\DIFaddend that sufficiently large reproduction in \DIFaddbegin \DIFadd{a }\DIFaddend single infection can guarantee single equilibrium when parasites coordinate (\DIFdelbegin \DIFdel{top left quadrant in Figure \ref{fig:manipbifur} Left at $f_w = (38)$, Figure SI.2 }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:manipbifur} B, D}\DIFaddend ).
In contrast, slightly reducing values of \DIFdelbegin \DIFdel{reproduction and manipulation in single infection increase }\DIFdelend \DIFaddbegin \DIFadd{either reproduction or manipulation in a single infection increases }\DIFaddend the bistability area \DIFaddbegin \DIFadd{(Figure \ref{fig:manipbifur}A, C, D)}\DIFaddend .
If the parasites sabotage each other, the system is highly prone to bistability and only has a single equilibrium when reproduction is \DIFdelbegin \DIFdel{especially enhanced(left side of vertical line in Figure \ref{fig:manipbifur} Left)}\DIFdelend \DIFaddbegin \DIFadd{enhanced}\DIFaddend .
Interestingly, \DIFdelbegin \DIFdel{sufficiently high reproduction enhancement leads to bistability (i.e. $f_{ww}$ is at least four times $f_w$), and }\DIFdelend reproductive incoordination, i.e. depressed reproduction and manipulative cooperation, always leads to a single equilibrium of the system (\DIFdelbegin \DIFdel{Figure \ref{fig:bistability}A, and bottom right quadrant in Figure \ref{fig:manipbifur}Left}\DIFdelend \DIFaddbegin \DIFadd{bottom right quadrants Figure \ref{fig:manipbifur}A, B}\DIFaddend ). 
...
\captionsetup{format=plain}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Figures/ratio_reproduction_manipulation_combi.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Figures/manip_bifur.pdf}
\DIFaddendFL \caption{\textbf{Effect of manipulation and reproduction on bistability.}
\DIFdelbeginFL \DIFdelFL{Left: $R_0$ values increase with more efficient manipulation in single and double infection.  
}\DIFdelendFL The \DIFdelbeginFL \DIFdelFL{hatched }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bistability }\DIFaddendFL area \DIFdelbeginFL \DIFdelFL{indicates }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(shaded areas) reduces as }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{bistable region. 
As }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{reproduction rate ($f_w$) and }\DIFaddendFL manipulation \DIFaddbeginFL \DIFaddFL{($\beta_w$) }\DIFaddendFL in single infection increases\DIFdelbeginFL \DIFdelFL{, the system only has one stable equilibrium}\DIFdelendFL . 
\DIFdelbeginFL \DIFdelFL{On the black line, the manipulation level is equal between }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Reproduction in }\DIFaddendFL single \DIFdelbeginFL \DIFdelFL{and double }\DIFdelendFL infection \DIFdelbeginFL \DIFdelFL{($\beta_w = \beta_{ww}$). 
In }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{decreases from }\DIFaddendFL the upper \DIFdelbeginFL \DIFdelFL{triangular area}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{panels (A}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{parasites cooperate, and in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{B) to }\DIFaddendFL the lower \DIFdelbeginFL \DIFdelFL{triangular area, parasites sabotage.
Right: Changes in the bistability area }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{panels }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{shaded areas}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{C, D}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{concerning different reproduction rates }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{while manipulation }\DIFaddendFL in single infection \DIFaddbeginFL \DIFaddFL{increases from the left panels }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{different boundary styles}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{A, C}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{to the right panels (B, D)}\DIFaddendFL . Manipulation and reproduction levels are equal between single and double infection on the vertical and horizontal lines. On the left side of the vertical line, $\beta_{ww} > \beta_{w}$, indicating cooperation, whereas on the right side of \DIFdelbeginFL \DIFdelFL{thevertical }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the vertical }\DIFaddendFL line, $\beta_{ww} < \beta_{w}$, indicating sabotage. On the upper part of the horizontal line, $f_{ww} > f_w$, indicating \DIFdelbeginFL \DIFdelFL{enhanded }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{enhanced }\DIFaddendFL reproduction, whereas, on the lower part of the horizontal line, $f_{ww} < f_w$, indicating suppressed reproduction.
...
First, extinction is more likely with varying levels of co-transmission from the parasite pool to \DIFaddbegin \DIFadd{the }\DIFaddend intermediate host, $p$, compared to varying levels of co-transmission from \DIFaddbegin \DIFadd{the }\DIFaddend intermediate host to \DIFaddbegin \DIFadd{the }\DIFaddend definitive host, $q$. 
For exceptionally high \DIFdelbegin \DIFdel{level }\DIFdelend \DIFaddbegin \DIFadd{levels }\DIFaddend of cooperation and \DIFdelbegin \DIFdel{not very small  values of both }\DIFdelend \DIFaddbegin \DIFadd{intermediate values of }\DIFaddend $p$ and $q$, the predator-prey-parasite system will always persist with one stable equilibrium.
...
However, this bistable area \DIFdelbegin \DIFdel{disapears }\DIFdelend \DIFaddbegin \DIFadd{disappears }\DIFaddend with high values of $q$ but not with high values of $p$. 
When parasites cooperate in manipulation, reducing $p$ almost always \DIFdelbegin \DIFdel{lead to bistabilitywhere as }\DIFdelend \DIFaddbegin \DIFadd{leads to bistability, whereas }\DIFaddend reducing $q$ can lead to \DIFaddbegin \DIFadd{a }\DIFaddend single equilibrium if cooperation is sufficiently large.  
Bistability indicates vulnerability to disturbance, \DIFdelbegin \DIFdel{and }\DIFdelend so cooperation in manipulation may be beneficial when $q$, the co-transmission from the intermediate host to the definitive host, decreases. 
However, cooperation in manipulation may still harm the population \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend reducing $p$, the co-transmission from the parasite pool to \DIFaddbegin \DIFadd{the }\DIFaddend intermediate host.
...
However, our model of a single intermediate host species \DIFdelbegin \DIFdel{can already provide }\DIFdelend \DIFaddbegin \DIFadd{already includes }\DIFaddend enough complexity to discuss the relationship between transmission and manipulation.
...
Moreover, the results of our ecological model are a baseline for further investigation of the evolution of host manipulation, where introducing the parasite pool may create interesting eco-evolutionary \DIFdelbegin \DIFdel{feedbacks }\DIFdelend \DIFaddbegin \DIFadd{feedback }\DIFaddend to the system.

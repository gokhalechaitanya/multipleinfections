...
%DIF DEL Oikos_submittedversion_round3.tex   Wed Nov 13 11:29:17 2024
%DIF ADD main.tex                            Thu Nov 14 10:35:54 2024
...
\DIFdelbegin %DIFDELCMD < \small{$^\ddag$Max Planck Institute for Evolutioanry Biology, Department of Theoretical Biology}%%%
\DIFdelend \DIFaddbegin \small{$^\ddag$Max Planck Institute for Evolutionary Biology, Department of Theoretical Biology}\DIFaddend \\
...
%DIFDELCMD < \textbf{Abstract:} 
%DIFDELCMD < Host manipulation is a common strategy of parasites with complex life cycle. 
...
%DIFDELCMD < Host manipulation, however, can also reduce predation due to conflicting interests when multiple parasites infect a host, which is often neglected in theoretical studies.
...
%DIFDELCMD < Including this realistic complexity in a mathematical model, the results depart from previous studies substantially. 
%DIFDELCMD < We show that coinfecting multi-trophic parasites can preserve the predator-prey system and themselves through manipulation and reproduction parameters.
%DIFDELCMD < Our study highlights the necessity of, and provides the means for, incorporating the reality of multiple parasites and their multi-trophic life cycles into the theory of parasite ecology.
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{One }\DIFdelend \DIFaddbegin \DIFadd{Host manipulation is a }\DIFaddend famous strategy that inspires many science fiction movies and novels\DIFdelbegin \DIFdel{is host manipulation}\DIFdelend , where a parasite can alter its host's morphology and/or behaviour to enhance its transmission to the next host \citep{Hughes2012}. 
...
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.5\columnwidth]{Figures/Sept_10.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\columnwidth]{Figures/cartoon2.pdf}
\DIFaddendFL \caption{\textbf{Who is in control?}
...
The presence of multiple parasites in the same host can lead to competition and strategic decisions \DIFdelbeginFL \DIFdelFL{pertaining to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{about }\DIFaddendFL investment in manipulation and growth.
...
\DIFaddbeginFL \DIFaddFL{While this is a specific example of a parasite with a complex life cycle, our model abstracts the concept to generic multi-host life cycles with an environmental component.
}\DIFaddendFL }
\label{fig:empirical}
\end{figure}
...
We include multiple infections \DIFaddbegin \DIFadd{of the same parasite species }\DIFaddend and consider the dynamics of the free-living parasite pool. 
...
For simplicity, we assume that hosts can be infected by one (single infection) or, at most, two parasites \DIFaddbegin \DIFadd{of the same species }\DIFaddend (double infections).
...
Typically, the number of parasites in multiple infections follows a negative binomial distribution, i.e. most hosts are infected with a few parasites\DIFdelbegin \DIFdel{while very }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend few hosts are infected with many parasites \citep{wilson:FE:1996}.
...
This assumption is motivated because the prey\DIFaddbegin \DIFadd{'s }\DIFaddend life cycle is often shorter than the predator's. 
...
\centering
\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{c|p{10cm}}%%%
%DIF < {|p{2.5cm}|p{12cm}|} 
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{p{2cm}|p{10cm}|p{3cm}}
\DIFaddendFL \hline
Parameters and Variables    &  Description \DIFaddbeginFL & \DIFaddFL{Dimensionality }\DIFaddendFL \\
\hline
$I_i$  & Density of intermediate hosts that are susceptible $i=s$, singly infected $i=w$, or doubly infected $i=ww$ \DIFaddbeginFL & \DIFaddFL{$[I_i] t^{-1}$}\DIFaddendFL \\
\hline
$D_i$ & Density of definitive hosts that are susceptible $i=s$, singly infected $i=w$, or doubly infected $i=ww$ \DIFaddbeginFL & \DIFaddFL{$[D_i]  t^{-1}$ }\DIFaddendFL \\
\hline
$W$ & Density of parasites released from definitive hosts into the environment \DIFaddbeginFL & \DIFaddFL{$[W]  t^{-1}$}\DIFaddendFL \\
\hline
\DIFaddbeginFL \DIFaddFL{$r$ }& \DIFaddFL{Reproduction rate of intermediate host }& \DIFaddFL{$t^{-1}$ }\\
\hline
\DIFaddFL{$k$ }& \DIFaddFL{Competition coefficient }& \DIFaddFL{$[I_i]^{-1}$ }\\
\hline
\DIFaddendFL $d$ & Natural death rate of intermediate hosts \DIFaddbeginFL & \DIFaddFL{$t^{-1}$}\DIFaddendFL \\
\hline
$\alpha_i$ & Additional death rate of intermediate hosts due to infection by a single parasite ($i = w$) or two parasites ($i = ww$) \DIFaddbeginFL & \DIFaddFL{$t^{-1}$ }\DIFaddendFL \\
\hline
$p$ & Probability that two parasites \DIFdelbeginFL \DIFdelFL{cotransmit }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{co-transmit }\DIFaddendFL from the environment to an intermediate host  \DIFaddbeginFL & \DIFaddFL{dimensionless}\DIFaddendFL \\
\hline
$\gamma$ & Transmission rate of parasites in the environment to intermediate hosts \DIFaddbeginFL &  \DIFaddFL{$[W]^{-1} t^{-1}$ }\DIFaddendFL \\
\hline
\DIFaddbeginFL \DIFaddFL{$\rho$ }& \DIFaddFL{Baseline capture rate }& \DIFaddFL{$[I_i]^{-1} t^{-1}$ }\\
\hline
\DIFaddFL{$c$ }& \DIFaddFL{Coefficient of energy conversion into new definitive host }& \DIFaddFL{dimensionless }\\
\hline
\DIFaddendFL $\mu$ & Natural death rate of definitive hosts \DIFaddbeginFL & \DIFaddFL{$t^{-1}$ }\DIFaddendFL \\
\hline
$\sigma_i$ & Additional death rate of definitive hosts due to infection by a single parasite ($i = w$) or two parasites ($i = ww$) \DIFaddbeginFL & \DIFaddFL{$t^{-1}$ }\DIFaddendFL \\
\hline
$\sigma_i$ & Additional death rate of the hosts due to being infected by a \DIFdelbeginFL \DIFdelFL{singly }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{single }\DIFaddendFL parasite ($i = w$) or two parasites ($i = ww$) \DIFaddbeginFL & \DIFaddFL{$t^{-1}$}\DIFaddendFL \\
\hline
$q$ & Probability that two parasites \DIFdelbeginFL \DIFdelFL{cotransmit }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{co-transmit }\DIFaddendFL from intermediate hosts to definitive hosts \DIFaddbeginFL & \DIFaddFL{dimensionless }\DIFaddendFL \\
\hline
$\beta_i$ & Transmission rate of parasites from intermediate hosts to definitive hosts \DIFaddbeginFL & \DIFaddFL{$[I_i]^{-1} t^{-1}$}\DIFaddendFL \\
\hline
$f_i$ & Reproduction rate of parasites in singly infected definitive hosts ($i = w$) or doubly infected hosts ($i = ww$) \DIFaddbeginFL &  \DIFaddFL{$t^{-1}$ }\DIFaddendFL \\
\hline
$\delta$ & Natural death rate of parasites in the environment \DIFaddbeginFL & \DIFaddFL{$t^{-1}$ }\DIFaddendFL \\
\hline 
$h$ & Probability that the parasites successfully established inside the definitive host \DIFaddbeginFL &  \DIFaddFL{dimensionless }\\
\hline
\multicolumn{3}{l}{$*$ $[I_i]$, $[D_i]$, and $[W]$ have the same unit $(individual \ area^{-1})$}
\DIFaddendFL \end{tabular}
\DIFdelbeginFL %DIFDELCMD < \bigskip{}
%DIFDELCMD < %%%
...
Here,  $R_0 > 1$  happens when $\gamma$, the transmission rate from the environment to intermediate hosts, and the reproduction rates $f_w, f_{ww}$ are quite large (as compared to the theoretical threshold shown by the mathematical conditions in \DIFdelbegin \DIFdel{SI3}\DIFdelend \DIFaddbegin \DIFadd{SI 3}\DIFaddend ). 
However, even when this condition is satisfied, the parasite may not be able to spread and persist in cyclic susceptible host dynamics (Figure \DIFdelbegin \DIFdel{SI1}\DIFdelend \DIFaddbegin \DIFadd{SI.1}\DIFaddend ). 
...
\end{align*}
%
where \DIFaddbegin \DIFadd{$r$ is the intrinsic growth rate of the intermediate hosts, and }\DIFaddend $k$ is the intraspecific competition coefficient. 
...
Here, as \DIFaddbegin \DIFadd{the }\DIFaddend reproduction and transmission \DIFdelbegin \DIFdel{value }\DIFdelend \DIFaddbegin \DIFadd{values }\DIFaddend of the parasite are not sufficient, it goes extinct (Figure \ref{fig:ecotraject:nonlinear}A, B), leaving the predator-prey dynamics attaining equilibrium (Figure \ref{fig:ecotraject:nonlinear}C, D)

\begin{figure}[!ht]
\DIFaddbeginFL \centering
\DIFaddendFL \captionsetup{format=plain}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\textwidth]{Figures/ecotraject_nonlinear.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.85\textwidth]{Figures/ecotraject_nonlinear.pdf}
...
Filled circles indicate stable \DIFdelbeginFL \DIFdelFL{equilibrium}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{equilibria}\DIFaddendFL , and open circles indicate unstable equilibrium. Parameter $\rho = 1.2, \  d = 0.9, \  r = 2.5, \ \gamma = 2.9, \ \alpha_w = 0, \ \alpha_{ww} =  0, \ \beta_w = 1.5, \ \beta_{ww} = 1.5, \ p = 0.05, \  c = 1.4, \ \mu = 3.9,  \ \sigma_w = 0, \ \sigma_{ww} = 0, \  q = 0.05, \ \delta = 0.9, \ k = 0.26, \ h = 0.6$.
}
...
Host manipulation can be cooperative; two parasites increase the predation rate on intermediate hosts, or $\beta_{ww} > \beta_w$ \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Hafer:2015gl}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . 
However, it can also be uncooperative; the predation rate on doubly-infected intermediate hosts is lower than that on singly-infected ones or $\beta_{ww} < \beta_w$ \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Hafer:2015gl}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
Cooperation in parasite manipulation increases the parasite's basic reproduction ratio $R_0$, but the manipulation in a single infection substantially affects the value of $R_0$ (Figure \ref{fig:R0}\DIFaddbegin \DIFadd{, see section SI 6 for analytical results}\DIFaddend ). 
...
\DIFdelbegin \DIFdel{When the }\DIFdelend \DIFaddbegin \DIFadd{This is due to the Allee effect \mbox{%DIFAUXCMD
\citep{stephens:OIKOS:1999} }\hspace{0pt}%DIFAUXCMD
where the parasite spreads and persists if, initially, there are sufficient parasites in the free-living pool as well as in the intermediate and definitive hosts.
On the contrary, if the initial populations of parasites are insufficient, the parasite will perish (Figure SI.2).
In addition, when the }\DIFaddend system encounters bistability, the parasite population risks extinction if there is a disturbance in the community.
...
Interestingly, reproductive incoordination, \DIFdelbegin \DIFdel{i.e. }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend depressed reproduction and \DIFaddbegin \DIFadd{sufficient }\DIFaddend manipulative cooperation, always leads to a single equilibrium of the system (bottom right quadrants Figure \ref{fig:manipbifur}\DIFdelbegin \DIFdel{A, B}\DIFdelend \DIFaddbegin \DIFadd{, note that if we extend the relative manipulation in Figure C and D, we also obtain single equilibrium in this area}\DIFaddend ). 
...
\DIFdelbegin \DIFdel{Finally, our study }\DIFdelend \DIFaddbegin \DIFadd{Our study specifically }\DIFaddend focuses on the ecological dynamics of a trophically transmitted parasite between two host species. 
...
\DIFaddbegin \DIFadd{In addition, we consider multiple infections of the same parasite species.
...
}\DIFaddend Here, we introduce more realistic features compared to previous models, such as a free-living parasite pool and multiple infections, regardless of some simplifications, such as multiple infections being limited to at most two parasites. 
\DIFdelbegin \DIFdel{In this way}\DIFdelend \DIFaddbegin \DIFadd{Thus}\DIFaddend , we can obtain analytical results of the reproduction ratio and mathematical expressions for the existing condition of the parasite.
\DIFaddbegin 

\DIFaddend Our model serves as a groundwork for future exploration into more complex and realistic systems, where numerical simulation may be the only possible approach.
\DIFdelbegin \DIFdel{Moreover, the }\DIFdelend \DIFaddbegin \DIFadd{Given that few studies considered measuring different parameters of trophically transmitted parasites \mbox{%DIFAUXCMD
\citep{Seppl2004, Gopko2015}}\hspace{0pt}%DIFAUXCMD
, our model calls for additional empirical work to measure relevant parameters, especially those from the parasite's perspective. 
...
Eventually, the }\DIFaddend results of our ecological model are a baseline for further investigation of the evolution of host manipulation, where introducing the parasite pool may create interesting eco-evolutionary feedback to the system.
...
%DIF < Funding from Bayerische Forschungsallianz and from the Max Planck Society if gratefully acknowledged (CSG).
%DIF > Funding from Bayerische Forschungsallianz and from the Max Planck Society is gratefully acknowledged (CSG).

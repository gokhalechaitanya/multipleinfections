...
%DIF DEL old.tex     Wed Jun  8 09:38:42 2022
%DIF ADD main2.tex   Wed Jun  8 09:29:37 2022
\usepackage{times}
...
\DIFdelbegin \DIFdel{Parasites }\DIFdelend \DIFaddbegin \DIFadd{Thus the parasites can move through multiple hosts and }\DIFaddend reproduce inside their definitive hosts \DIFdelbegin \DIFdel{and are }\DIFdelend \DIFaddbegin \DIFadd{before being }\DIFaddend released into the environment. 
\DIFdelbegin \DIFdel{They infect intermediate hosts when the hosts encounter the parasite pool.
When }\DIFdelend \DIFaddbegin \DIFadd{The parasites pass through the intermediate hosts between the environment and the definitive host.
When a definitive host consumes }\DIFaddend an infected intermediate host\DIFdelbegin \DIFdel{is consumed by a definitive host}\DIFdelend , the definitive host gets infected\DIFaddbegin \DIFadd{, }\DIFaddend and the parasite \DIFdelbegin \DIFdel{complete }\DIFdelend \DIFaddbegin \DIFadd{completes }\DIFaddend its lifecycle.
...
\DIFdelbegin \DIFdel{Assuming that infection always happens whenever there are encounters between parasites and uninfected hosts, $p$ is the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend probability that two parasites in the parasite pool co-transmit to an intermediate host \DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{is denoted by  $p$, and thus }\DIFaddend $1-p$ is the probability that a single \DIFdelbegin \DIFdel{parasites }\DIFdelend \DIFaddbegin \DIFadd{parasite }\DIFaddend enters an intermediate host. 
\DIFaddbegin \DIFadd{This formulation assumes that infection always happens whenever there are encounters between parasites and uninfected hosts.
}\DIFaddend When a definitive host consumes an intermediate host infected by two parasites, there is a probability $q$ that both parasites co-transmit to the definitive host\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{.
With probability }\DIFaddend $1-q$\DIFdelbegin \DIFdel{is the probability that }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend only one parasite successfully transmits. 
...
%DIFDELCMD < %%%
\DIFdelend %DIF > 
where $R(I_s, I_w, I_{ww})$ represents the birth rate of the intermediate hosts, which is a function of both infected and \DIFdelbegin \DIFdel{noninfected individuals. 
In other words, all types of intermediate hosts can reproduce. 
}\DIFdelend \DIFaddbegin \DIFadd{uninfected individuals. 
%DIF > In other words, all types of intermediate hosts can reproduce. 
}\DIFaddend $\Pi_i$, where $i = \{s, w, ww\}$ is the predation function of definitive hosts on \DIFdelbegin \DIFdel{respectively, }\DIFdelend susceptible, singly infected and doubly infected intermediate hosts \DIFaddbegin \DIFadd{respectively}\DIFaddend . 
...
In particular, if \DIFdelbegin \DIFdel{an intermediate host is infected by }\DIFdelend a single parasite \DIFaddbegin \DIFadd{infects an intermediate host}\DIFaddend , the manipulation strategy is $\beta_w$\DIFdelbegin \DIFdel{, and if it is infected by two parasites }\DIFdelend \DIFaddbegin \DIFadd{. 
If two parasites infect it}\DIFaddend , the manipulation strategy is $\beta_{ww}$. 
\DIFdelbegin \DIFdel{There is a }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend link between $\beta_w$ and $\beta_{ww}$, \DIFdelbegin \DIFdel{but we need not specify the link for now. 
$\eta_w = \gamma W$ is the }\DIFdelend \DIFaddbegin \DIFadd{is explored further. 
The }\DIFaddend force of infection by parasites in the environment \DIFdelbegin \DIFdel{. 
$\lambda_i = \beta_i I_i$, where $i = \{ w, ww\}$, is the }\DIFdelend \DIFaddbegin \DIFadd{is denoted by $\eta_w = \gamma W$. 
The }\DIFaddend force of infection that corresponds respectively to singly infected intermediate host ($I_w$), or doubly infected intermediate hosts ($I_{ww}$) \DIFaddbegin \DIFadd{is denoted by $\lambda_i = \beta_i I_i$, where $i = \{ w, ww\}$}\DIFaddend . 
...
\end{align}
%
where $B(D_s, D_w, D_{ww}, I_s, I_w, I_{ww})$ represents the birth rate of definitive hosts, which is a function of population density of both intermediate and definitive hosts, infected or \DIFdelbegin \DIFdel{non-infected }\DIFdelend \DIFaddbegin \DIFadd{uninfected }\DIFaddend alike. 

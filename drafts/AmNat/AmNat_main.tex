...
\section*{Discussion}
Host manipulation is an ubiquitous phenonmenon and is suggested to affect the prey-predator dynamics in trophically transmitted parasites. In particular, manipulation in infected intermediate hosts to increase the predation rate of definitive hosts is suggested to result in a heavy burden on the intermediate host population, resulting in the predator-prey system and thus the parasite being more vulnerable to extinction (\ref{Hadeler1989, Fenton2006}). 


Our model shows that parasites in general can hardly spread in a cyclic predator-prey system. This is an expected result because even though the basic reproduction ratio $R_0$ of the parasite is greater than 1, it is estimated at the unstable equilibrium (or cyclic equilibrium) of the predator and prey. This means that when the density of the prey and predator is at the minimum value of the cycle, the ''effective'' $R_0$ of the parasite can be smaller than one. This result does not contradict that of \cite{Rogawa2018} as in their work, the author shows that the parasite can spread in a stable prey-predator dynamic and then cause the system to cycle. We however find that the parasite does not necessarily destabilise the system. This may be due to the introduction of a parasite pool and multiple infections. In \cite{Rogawa2018}, transmission from definitive host to intermediate host was assumed to be the result of direct contact between the two hosts and such immediate transmission could lead to rapid consumption of predator, hence, faster predator-prey dynamics, which could lead to cycle dynamics when parasites are introduced.


Host manipulation can destabilise our system under very specific circumstances and in a different way than previous models. In particular, the destabilisation of the system lies in the fact that there is bistability when parasite reproduction in coinfection is boosted. In this bistability region, if the system is disturbed (e.g. migration of the intermediate or definitive hosts, or predation of intermediate hosts by other predators), then the density of the infected hosts may crash, leading to parasite extinction. The bistability region is widen as the manipulation in double infection increases and manipulation in single infection is not sufficient. This is because the density of the doubly infected hosts is always much smaller than the density of the singly infected hosts because it is limited by sequential transmission and a small probability of cotransmission. If manipulation in single infection is not sufficient, transmission of the parasites depends largely on the double infection hosts, which is rare, and so extinction can happen if manipulation in double infection is not sufficiently high. We show that

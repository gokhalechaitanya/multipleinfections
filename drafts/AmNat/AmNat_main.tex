...
The parasite then transmits into the susceptible definitive host and eventually reproduces. 
In a disease-free environment, parasites are so rare that the reproduction ratio compartments with sequential infections are negligible.
...
We assume the same parasite virulence for double infections and single infection($\alpha_w = \alpha_{ww}$, $\sigma_w = \sigma_{ww}$), and reproduction in single infection as a linear function concerning reproduction in double infections ($f_w = \epsilon f_{ww}$).
We found that the parasite can establish and spread in the hosts if its reproduction value in single infection $f_w$ is more significant than a threshold (Figure \ref{fig:bistability}, SI). 
...
The parasite population will crash if disturbed and becomes too small (Figure \ref{fig:bistability}A, B). 
...
Within the bistable region, the basic reproduction ratio is less than one, suggesting that co-infected parasites that are cooperative at an intermediate level have a weak manipulative ability when alone. 
...
\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{Figures/manip_bifur_R0.pdf}
...
Cooperation between parasites need not be limited to host manipulation. Parasites can cooperate to have a higher reproduction rate in co-infections. 
Our model explicitly indicates that parasites in doubly infected definitive hosts have higher reproduction rates than those in singly infected definitive hosts. 
Without any assumption on the relationship between manipulative ability and reproduction, parasites can cooperate in both manipulation and reproduction. 
...
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.3]{Figures/manip_bifurcation.pdf}
...
In particular, manipulation in infected intermediate hosts to increase the predation rate of definitive hosts is suggested to result in a heavy burden on the intermediate host population, resulting in the predator-prey system and thus the parasite being more vulnerable to extinction \cha{(\ref{Hadeler1989, Fenton2006})}. 


Our model shows that parasites generally can hardly spread in a cyclic predator-prey system. 
This is an expected result because even though the parasite's basic reproduction ratio $R_0$ is greater than 1, it is estimated at the unstable equilibrium (or cyclic equilibrium) of the predator and prey. 
...
This result does not contradict that of \cite{Rogawa2018}. 
In their work, the author shows that the parasite can spread in a stable prey-predator dynamic and cause the system to cycle. 
We, however, find that the parasite does not necessarily destabilise the system. 
This may be due to the introduction of a parasite pool and multiple infections. 
In \cite{Rogawa2018}, transmission from definitive host to intermediate host was assumed to be the result of direct contact between the two hosts, and such immediate transmission could lead to rapid predator consumption. 
Hence, faster predator-prey dynamics occur, which could lead to cyclic dynamics when parasites are introduced.


Host manipulation can destabilise our system under particular circumstances and in a different way than previous models. 

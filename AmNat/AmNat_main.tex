...
The parasite completes its lifecycle when a definitive host becomes infected after consuming an infected intermediate host.
...
This formulation assumes that infection always happens hosts encounter parasites.
...
The expression of $R_0$ indicates the possible reproduction routes of a parasite, which can be via double or single infections. 
...
In a disease-free environment, parasites are so rare that the reproduction ratio compartments with sequential infections do not appear. 


If $R_0 > 1$, a parasite can spread when introduced into the disease-free equilibrium of prey and predator. 
This disease-free equilibrium exists regardless of the explicit form of the predation functions $Pi_w$ and $Pi_{ww}$. 
To understand the effect of manipulation on the fitness of parasites and the ecological dynamics of the system, we need to specify the predation functions and know the explicit form of the equilibrium $I_s^*$ and $D_s^*$. 
The explicit forms of $I_s^*$ and $D_s^*$ depend primarily on the birth functions $R$ and $B$ of, respectively, the intermediate and definitive host and on the predation functions $\Pi_s, \Pi_w, \Pi_{ww}$.
...
We consider the system when the birth function $R$ of the intermediate host is linear, specifically, $R(I_s, I_w, I_ww) = r(I_s + I_w + I_ww)$. 
